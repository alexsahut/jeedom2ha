...
/* Story 16.5 — Onglet « HA / jeedom2ha » : triptyque override par commande.
 * Toute la logique métier (état diagnostic, auto-validation, réassurance) vit dans
 * le module pur Jeedom2haMappingOverride ; ici uniquement le câblage jQuery/AJAX. */
(function () {
  ...
  var DEBOUNCE_MS = 400;
  var overrideCtx = { eqId: null, reassurance: M.initReassuranceState(), timers: {}, xhr: {} };

  function currentEqId() {
    var $id = $('.eqLogicAttr[data-l1key=id]');
    if ($id.length === 0) {
      return null;
    }
    var raw = (typeof $id.value === 'function') ? $id.value() : $id.val();
  ...
  function renderDiagnosticCell($cell, view) {
    ...
    var msg = M.buildDiagnosticMessage(view);
  ...
  function maybeShowReassurance(isBlocking) {
    if (M.shouldShowReassurance(overrideCtx.reassurance, isBlocking)) {
      overrideCtx.reassurance = M.markReassuranceShown(overrideCtx.reassurance);
      $('#mappingOverride_reassurance').show();
    }
  }

  function renderCommandRow(eqId, row) {
    var cmdId = row.jeedom_cmd_id;
    var bodyId = 'moBody_' + eqId + '_' + cmdId;
    var headId = 'moHead_' + eqId + '_' + cmdId;
    var $item = $('<div class="panel panel-default mapping-override-cmd"></div>').attr('data-cmd-id', cmdId);

    // En-tête accordéon Bootstrap (collapsible) — titre = commande + natif D10.
    var $head = $('<div class="panel-heading" role="tab"></div>').attr('id', headId);
    var $toggle = $('<a class="mo-accordion-toggle" role="button" data-toggle="collapse"></a>')
      .attr('href', '#' + bodyId)
      .attr('aria-expanded', 'false')
      .attr('aria-controls', bodyId);
    $toggle.append($('<span class="mo-cmd-name"></span>').text(row.cmd_name || ('#' + cmdId)));
    $toggle.append($('<span class="mo-native-value text-muted" aria-readonly="true"></span>').text(' · ' + (row.generic_type || '—')));
    $head.append($('<h4 class="panel-title"></h4>').append($toggle));
    $item.append($head);

    // Corps accordéon replié par défaut ; contient le triptyque.
    var $collapse = $('<div class="panel-collapse collapse" role="tabpanel"></div>')
      .attr('id', bodyId)
      .attr('aria-labelledby', headId);
    var $body = $('<div class="panel-body mapping-override-triptyque"></div>');

    // Colonne natif (lecture seule, D10) — visuellement distincte.
    var $native = $('<div class="mo-col mo-col-native"></div>');
    $native.append($('<div class="mo-col-label"></div>').text('{{Natif Jeedom (partagé Homebridge)}}'));
    $native.append($('<span class="mo-native-value" aria-readonly="true"></span>').text(row.generic_type || '—'));
    $native.append($('<span class="mo-cmd-name text-muted"></span>').text(' ' + (row.cmd_name || '')));

    // Colonne override (éditable).
    var $override = $('<div class="mo-col mo-col-override"></div>');
    $override.append($('<div class="mo-col-label"></div>').text('{{Override Home Assistant}}'));
    var $select = buildOptions(row.override_applied ? row.effective_ha : '');
    $override.append($select);
    if (!row.override_applied) {
      $override.append($('<div class="mo-empty-state text-muted"></div>').text(M.buildEmptyStateLabel(row)));
    }
    var $spinner = $('<span class="mo-spinner" style="display:none;"><i class="fas fa-spinner fa-spin"></i></span>');
    $override.append($spinner);
    if (row.override_applied) {
      $override.append($('<a class="mo-revert-cmd cursor"><i class="fas fa-undo"></i> {{Mode automatique}}</a>'));
    }

    // Colonne diagnostic.
    var $diag = $('<div class="mo-col mo-col-diag"></div>');
    $diag.append($('<div class="mo-col-label"></div>').text('{{Diagnostic}}'));
    var $diagCell = $('<div class="mo-diag-cell"></div>');
    renderDiagnosticCell($diagCell, row.diagnostic);
    $diag.append($diagCell);

    $body.append($native).append($override).append($diag);
    $collapse.append($body);
    $item.append($collapse);

    // Débounce dry-run instantané + auto-validation (AC7/8/9).
    $select.on('change', function () {
      var cmdId = row.jeedom_cmd_id;
      var type = $(this).val();
      if (overrideCtx.timers[cmdId]) {
        clearTimeout(overrideCtx.timers[cmdId]);
      }
      if (!type) {
        // Retour explicite au mode auto par commande.
        revertCommand(eqId, cmdId);
        return;
      }
      overrideCtx.timers[cmdId] = setTimeout(function () {
        runDryRun(eqId, cmdId, type, $diagCell, $spinner);
      }, DEBOUNCE_MS);
    });

    $item.find('.mo-revert-cmd').on('click', function () {
      revertCommand(eqId, row.jeedom_cmd_id);
    });
  ...
  function runDryRun(eqId, cmdId, type, $diagCell, $spinner) {
    // Annulation de la requête en vol pour cette commande (race condition).
    if (overrideCtx.xhr[cmdId]) {
      overrideCtx.xhr[cmdId].abort();
    }
    $spinner.show();
    overrideCtx.xhr[cmdId] = $.ajax({
      type: 'POST',
      url: AJAX_URL,
      ...
      success: function (data) {
        ...
        var blocking = M.isBlockingDiagnostic(view);
        maybeShowReassurance(blocking);
  ...
  function renderTree(payload) {
    var tree = M.normalizeTree(payload);
    var eqId = tree.jeedom_eq_id;
    var $list = $('#mappingOverride_list').empty();
    if (!tree.mapped && tree.commands.length === 0) {
      $('#mappingOverride_status').text('{{Aucune commande à configurer pour cet équipement.}}');
      $('#mappingOverride_eqActions').empty();
      return;
    }
    $('#mappingOverride_status').text('');

    // Action de retour au mode auto pour tout l'équipement (AC10 / subtask 4.1).
    var hasOverride = false;
    for (var k = 0; k < tree.commands.length; k++) {
      if (tree.commands[k].override_applied) {
        hasOverride = true;
        break;
      }
    }
    var $eqActions = $('#mappingOverride_eqActions').empty();
    if (hasOverride) {
      var $revertEq = $('<a class="mo-revert-eq cursor btn btn-default btn-sm"><i class="fas fa-undo"></i> ' +
        '{{Revenir au mode automatique (tout l’équipement)}}</a>');
      $revertEq.on('click', function () {
        revertEquipment(eqId);
      });
      $eqActions.append($revertEq);
    }

    for (var i = 0; i < tree.commands.length; i++) {
      $list.append(renderCommandRow(eqId, tree.commands[i]));
    }

    // Auto-ouverture des commandes bloquantes détectées au GET, plafonnée (subtask 3.4).
    var blockingIds = M.collectBlockingCommandIds(tree, 4);
    for (var b = 0; b < blockingIds.length; b++) {
      var $panel = $list.find('.mapping-override-cmd[data-cmd-id="' + blockingIds[b] + '"]');
      $panel.find('.panel-collapse').addClass('in').attr('aria-hidden', 'false');
      $panel.find('.mo-accordion-toggle').attr('aria-expanded', 'true');
    }
    if (blockingIds.length > 0) {
      // Focus clavier cohérent : première commande bloquante ouverte.
      $list.find('.mapping-override-cmd[data-cmd-id="' + blockingIds[0] + '"]')
        .find('.mo-accordion-toggle').first().attr('tabindex', '-1').focus();
    }
  }
  ...
  function loadOverrideTree(eqId) {
    $('#mappingOverride_status').text('{{Chargement…}}');
    $.ajax({
      type: 'POST',
      url: AJAX_URL,
      ...
      success: function (data) {
        ...
        renderTree(result && result.payload ? result.payload : result);
      },
      error: function () {
        $('#mappingOverride_status').text('{{Impossible de charger la configuration HA (daemon indisponible).}}');
      },
    });
  }

  // Chargement paresseux : un seul GET à l'activation de l'onglet (jamais 284 d'un coup).
  $(document).on('shown.bs.tab', 'a[href="#mappingOverrideTab"]', function () {
    var eqId = currentEqId();
    if (eqId === null) {
      $('#mappingOverride_status').text('{{Enregistrez d’abord l’équipement pour configurer le mapping HA.}}');
      return;
    }
    overrideCtx.eqId = eqId;
    overrideCtx.reassurance = M.initReassuranceState();
    $('#mappingOverride_reassurance').hide();
    loadOverrideTree(eqId);
  });
})();
