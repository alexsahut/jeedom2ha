...
- [ ] `projection_validity` est présent dans le contrat 4D par ajout additif pur.
- [ ] Les cas nominal et skipped sont couverts par des tests automatisés.
- [ ] La non-régression complète du schéma et du diagnostic est démontrée.
- [ ] Aucun changement n'affecte `cause_action`.
- [ ] Aucun changement n'affecte `PRODUCT_SCOPE`.
- [ ] Aucune promesse d'action utilisateur n'est ajoutée.
- [ ] Si du PHP ou du JS est touché, il reste en lecture stricte uniquement.
- [ ] Aucun invariant pipeline critique n'est cassé.
- [ ] `projection_validity` est uniquement issu du pipeline, sans aucune simulation.
- [ ] La cohérence entre `projection_validity` et la décision est validée (AC8).
- [ ] Aucun code introduit ne modifie le comportement de `decide_publication()`.
- [ ] La story est prête à permettre 7.2 ensuite, sans ambiguïté de périmètre.
