...
- [ ] 0.1 — Lire `resources/daemon/models/cause_mapping.py` dans son intégralité
- [ ] 0.2 — Dresser l'inventaire exhaustif de `_REASON_CODE_TO_CAUSE` : lister chaque `reason_code` avec son triplet exact `(cause_code, cause_label, cause_action)` — voir Dev Notes §Inventaire de référence pour la baseline attendue
- [ ] 0.3 — Pour chacun des 5 codes AR8, noter s'il est **présent** ou **absent** dans la table :
  - `ha_missing_command_topic` → présent / absent
  - `ha_missing_state_topic` → présent / absent
  - `ha_missing_required_option` → présent / absent
  - `ha_component_unknown` → présent / absent
  - `ha_component_not_in_product_scope` → présent / absent
- [ ] 0.4 — Lire `resources/daemon/tests/unit/test_cause_mapping.py` et identifier les reason_codes qui ont un test et ceux qui n'en ont pas — voir Dev Notes §Gap de test de référence
- [ ] 0.5 — Lire `resources/daemon/validation/ha_component_registry.py` : `_REASON_PRIORITY`, `_CAPABILITY_TO_REASON`, `validate_projection()` — comprendre dans quel contexte chaque code classe 2 est produit
- [ ] 0.6 — **Vérifier le snapshot de référence** : comparer l'inventaire établi en 0.2 avec le `_BASELINE_SNAPSHOT` fourni dans Dev Notes §Snapshot de non-régression — ce snapshot est l'oracle contractuel, il n'est pas à reconstruire ; si une divergence est constatée (clé manquante, valeur différente, clé supplémentaire) : **STOP** — noter l'écart dans `Completion Notes` et ne pas démarrer Task 1 tant que l'écart n'est pas expliqué et résolu
- [ ] 0.7 — **Valider les règles contractuelles** : confirmer explicitement dans `Completion Notes` que aucun code existant ne sera modifié dans ce qui suit
...
- [ ] 1.1 — Pour chaque code trouvé absent par Phase 0, ajouter l'entrée selon le mapping prescriptif de Dev Notes §Mapping prescriptif AR8
- [ ] 1.2 — Vérifier que les clés et les valeurs des entrées préexistantes n'ont pas été modifiées (l'ordre des entrées dans le dict n'est pas contractuel — seules les clés et leurs triplets le sont)
- [ ] 1.3 — Vérifier que la logique de `reason_code_to_cause()` et `build_cause_for_pending_unpublish()` n'a pas été modifiée
- [ ] 1.4 — Mettre à jour le commentaire d'en-tête du fichier pour refléter le nouveau total d'entrées actives
...
- [ ] 2.1 — Ajouter dans `test_cause_mapping.py` la fonction `test_non_regression_snapshot()` — voir Dev Notes §Snapshot de non-régression pour le code exact
- [ ] 2.2 — Ajouter les tests individuels des codes trouvés sans test par Phase 0 (baseline attendue : `excluded_eqlogic`, `excluded_plugin`, `excluded_object`) — voir Dev Notes §Tests de non-régression individuels
...
- [ ] 3.1 — `test_ha_missing_command_topic()` — vérification du triplet exact conforme au mapping prescriptif
- [ ] 3.2 — `test_ha_missing_state_topic()` — idem
- [ ] 3.3 — `test_ha_missing_required_option()` — idem
- [ ] 3.4 — `test_ha_component_unknown()` — triplet exact, `cause_action is None` explicitement vérifié
- [ ] 3.5 — Si `ha_component_not_in_product_scope` était absent et a été ajouté : `test_ha_component_not_in_product_scope_additive()` — triplet exact conforme au mapping prescriptif

---

### Task 4 — Tests anti-contrat (AC: #4, #5)

- [ ] 4.1 — `test_unknown_code_contractual_return()` — un code inexistant retourne `(None, None, None)` sans exception et sans redirection vers un autre triplet (voir Dev Notes §Tests anti-contrat)
- [ ] 4.2 — `test_no_partial_mapping_in_table()` — boucle sur `_REASON_CODE_TO_CAUSE`, vérification structurelle : `cause_code` non-null implique `cause_label` non-null, pas de mapping partiel (voir Dev Notes §Tests anti-contrat)
- [ ] 4.3 — `test_classe2_codes_distinct_from_mapping_failures()` — **à exécuter après Task 1** : pour chaque code classe 2 présent dans `_REASON_CODE_TO_CAUSE`, vérifier que `cause_code` n'est pas `"no_mapping"` ni `"ambiguous_skipped"` — le test commence par asserter la présence du code dans la table pour éviter un faux négatif si Task 1 n'a pas été exécutée (voir Dev Notes §Tests anti-contrat)
- [ ] 4.4 — `test_ha_component_not_in_product_scope_maps_to_not_in_product_scope()` — vérifier que `cause_code = "not_in_product_scope"` (non-régression / conformité AR8, que le code ait été ajouté par 4.2 ou par cette story)

---

### Task 5 — Non-régression globale (AC: #6)

- [ ] 5.1 — `python3 -m pytest resources/daemon/tests/unit/test_cause_mapping.py -v` → 0 failure ; compter et noter le total de tests (attendu ≥ 28)
- [ ] 5.2 — `python3 -m pytest tests/ resources/daemon/tests/ -q` → 0 failure, 0 error ; noter le total
