...
- 2026-06-19 — Fix review PR #127 (codex P2) : le chemin action « publier » (`_publish_mapping_for_action`) ne publiait que le switch primaire ; les 12 sensors + 1 binary_sensor (`additional_mappings`) restaient non publiés lors d'une re-inclusion sans sync complet (entités manquantes côté HA). Étendu pour publier les secondaires via dispatch dédié + test de régression. 917 tests verts.
