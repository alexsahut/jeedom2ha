...
### Epic 8 — Le dispatch et la publication deviennent registry-driven sans changer le comportement ouvert

**Valeur utilisateur :** Le produit peut continuer a evoluer vers de nouveaux types HA sans re-casser les surfaces deja ouvertes. L'utilisateur ne voit pas de regression fonctionnelle pendant que le moteur gagne en extensibilite.

**Résultat observable :** Le dispatch des mappers, la table de publication et la boucle sync deviennent registry-driven avec parite comportementale stricte sur les types deja ouverts. Un golden-file de reference verrouille toute regression avant l'ouverture des vagues suivantes.

**FRs couverts :** FR15, FR26, FR27, FR41, FR45

**ARs clés :** AR1, AR3, AR10, AR13

**NFRs directement adressés :** NFR1, NFR10, NFR11, NFR12

**Invariants à porter en stories :**
- aucune story n'introduit de nouveau type publie cote HA ;
- aucune story ne modifie `PRODUCT_SCOPE` ;
- toute modification de dispatch est gardee par le golden-file de reference ;
- le comportement sur `light`, `cover`, `switch` reste strictement identique.

---

### Epic 9 — La vague 1 ouvre reellement `sensor`, `binary_sensor` et `button`, avec degradation elegante terminale

**Valeur utilisateur :** L'utilisateur retrouve enfin dans Home Assistant des capteurs simples, des capteurs binaires et des actions ponctuelles qui etaient jusque-la connues du moteur mais non publiees reellement.

**Résultat observable :** `sensor`, `binary_sensor` et `button` sont mappes, valides, publies et traces de bout en bout. La regle de degradation elegante §11 est enfin cablee par `FallbackMapper`, et les CTA restent limites aux surfaces reellement ouvertes.

**FRs couverts :** FR11, FR16, FR26, FR32, FR39, FR40, FR42

**ARs clés :** AR3, AR4, AR11, AR13

**NFRs directement adressés :** NFR2, NFR5, NFR10, NFR11, NFR12

**Invariants à porter en stories :**
- aucune story n'introduit de type hors vague 1 ;
- `button` est la seule extension de `PRODUCT_SCOPE` de cet epic et reste gouvernee par FR40 / NFR10 ;
- le `FallbackMapper` n'ouvre aucun type nouveau, il ne s'appuie que sur des types deja ouverts ;
- la regle Epic 6 `no faux CTA` reste verrouillee sur toute la vague.

---

### Epic 10 — La compatibilite minimale Homebridge utile devient une vague HA gouvernee et traçable

**Valeur utilisateur :** L'utilisateur retrouve dans Home Assistant, de facon progressive et explicable, les usages Jeedom deja prouves via Homebridge/HomeKit, sans ouvrir de perimetre annexe ni perdre la gouvernance stricte du moteur.

**Résultat observable :** Un prefixe d'inventaire fige le perimetre Homebridge utile vise, puis la vague suivante ouvre uniquement les composants HA effectivement necessaires. La priorisation ne part plus d'un type HA arbitraire mais du delta reel Homebridge -> HA deja audite.

**FRs couverts :** FR11, FR16, FR26, FR31, FR39, FR40, FR42, avec reutilisation borne de FR32 / FR33 uniquement lorsqu'une remediation reelle existe

**ARs clés :** AR3, AR4, AR6, AR11, AR13

**NFRs directement adressés :** NFR2, NFR5, NFR10, NFR11, NFR12

**Invariants à porter en stories :**
- `ha-projection-reference.md` reste la source-of-truth des contraintes HA ; Homebridge ne sert qu'a prioriser la vague ;
- aucune plateforme annexe (`camera`, `samsung`, `alexa`, `gsh`, etc.) n'entre dans `pe-epic-10` ;
- toute ouverture de type dans `PRODUCT_SCOPE` passe dans le meme increment avec ses preuves FR40 / NFR10 ;
- la parite visee se lit en familles d'usage et exemples representatifs, pas en dump exhaustif non gouverne ;
- aucune equivalence UX metier sensible (`presence`, composites complexes, etc.) n'est ouverte cosmetiquement sans intention produit explicite.
...
## Epic 10 — Vague Homebridge -> HA minimale utile : inventaire cible, scenarios HomeKit, climate, alarme et composites metier gouvernes

`pe-epic-9` a cloture la premiere vague d'extension reelle (`sensor`, `binary_sensor`, `button`) mais n'a pas encore comble le delta entre le perimetre deja prouve via Homebridge/HomeKit et ce qui est effectivement recouvert dans `jeedom2ha`. Cet epic ouvre une **vague de parite minimale utile** : on part de l'audit Homebridge deja archive (`homebridge-homekit-vs-ha-delta-2026-06-07.md`), on exclut explicitement les plateformes annexes, puis on derive les ouvertures HA strictement necessaires a partir de `ha-projection-reference.md`.

**Pre-requis** : `pe-epic-9` clos ; `homebridge-homekit-vs-ha-delta-2026-06-07.md` archive ; `sprint-change-proposal-2026-06-07.md` approuve comme correct-course de cadrage.

**Source de verite** : `_bmad-output/planning-artifacts/ha-projection-reference.md` pour les contraintes HA ; `_bmad-output/planning-artifacts/homebridge-homekit-vs-ha-delta-2026-06-07.md` pour la priorisation Homebridge -> HA ; `_bmad-output/planning-artifacts/sprint-change-proposal-2026-06-07.md` pour le sequencing cible.

**Cadre** : `pe-epic-10` ne convertit pas Homebridge en source de verite technique. Homebridge sert uniquement a dire **quoi couvrir en premier**. Les payloads, validations et ouvertures reelles restent gouvernes par le registre HA, `validate_projection()`, `PRODUCT_SCOPE`, FR40 et NFR10. Les plateformes annexes (`camera`, `SamsungTizen`, `homebridge-alexa`, `homebridge-gsh`) restent hors-scope. Les zones sensibles (`presence`) ou metier non 1:1 (`IQ EV`, `SPA`) ne sont ouvertes qu'apres cadrage explicite.

### Story 10.0 : Prefixe de gel du perimetre Homebridge -> HA vise

En tant que mainteneur,
je veux figer dans un artefact de travail executable la matrice `usage Homebridge -> generic types Jeedom -> composant HA cible -> statut actuel du moteur`,
afin que `pe-epic-10` parte d'un delta reel et partage plutot que d'une hypothese de type HA a ouvrir.

**Acceptance Criteria :**

**Given** l'audit archive dans `homebridge-homekit-vs-ha-delta-2026-06-07.md`
**When** la story de prefixe est executee
**Then** elle produit une matrice lisible couvrant au minimum :
...
**When** elle est inscrite dans la matrice
**Then** la ligne documente explicitement :
...
**Given** une plateforme annexe ou un usage explicitement exclu (`camera`, `samsung`, `alexa`, `gsh`, monitoring faible valeur)
...
**Dev notes :**
- story documentaire de prefixe obligatoire avant toute ouverture technique
- source terrain = audit transmis par `ClawBox`, pas de nouveau scan
- sert de base de preuve pour toutes les stories suivantes de `pe-epic-10`
...
**When** le gate terrain est execute sur la box reelle
...
**Dev notes :**
- cette story peut etre purement de verification si le repo couvre deja tout le besoin ; dans ce cas elle doit le prouver et clore l'ecart proprement
- aucun nouveau type HA n'est ouvert dans cette story
- toute action exposee reste conforme a la regle `no faux CTA`

---

### Story 10.2 : Ouverture gouvernee de `climate` pour les thermostats Homebridge representatifs

En tant qu'utilisateur,
je veux retrouver dans Home Assistant les thermostats deja exposes via Homebridge,
afin que la compatibilite minimale utile couvre aussi le chauffage et les consignes principales.

**Acceptance Criteria :**

**Given** les thermostats representatifs identifies par l'audit (`Thermostat chambre Arthur`, `Thermostat chambre Margaux`, `Thermostat chambre parent`, `Thermostat Galerie`, `Thermostat RDC`, `Thermostat SDB`, `Thermostat SPA`)
**When** la story est cadrée contre `ha-projection-reference.md`
**Then** les contraintes `climate` necessaires sont citees explicitement depuis la reference HA

**Given** un eqLogic Jeedom thermostat correctement type
**When** le mapper `climate` est invoque
**Then** il produit un `MappingResult` `ha_entity_type = "climate"` structurellement valide sur les cas nominaux representatifs
...
**Dev notes :**
- premiere vraie ouverture de type nouveau de `pe-epic-10`
- la story doit rester bornee a la compatibilite minimale utile, pas a l'equivalence HVAC exhaustive
- `Thermostat SPA` peut etre garde comme cas representatif mais ne doit pas imposer a lui seul la generalisation de composites metier

---

### Story 10.3 : Evaluation puis ouverture ciblee de `alarm_control_panel` pour `Alarme maison`

En tant qu'utilisateur,
je veux retrouver dans Home Assistant l'alarme deja visible via Homebridge lorsque la projection est structurellement et produitement honnete,
afin que la parite minimale couvre aussi ce point d'usage central sans ouvrir un faux miroir.

**Acceptance Criteria :**

**Given** le cas representatif `Alarme maison` dans la matrice 10.0
**When** la story est executee
**Then** elle tranche explicitement entre :
**And** `ouvrable en alarm_control_panel` ;
**And** `non ouvrable dans cette vague` ;
**And** `necessite un recadrage produit`

**Given** le verdict `ouvrable en alarm_control_panel`
**When** l'implementation est realisee
**Then** elle suit integralement la checklist canonique d'ouverture de type :
**And** contraintes de registre ;
**And** mapper ;
**And** publisher ;
**And** validation ;
**And** PRODUCT_SCOPE ;
...
**Given** le verdict `non ouvrable` ou `recadrage produit`
**When** la story est close
**Then** le diagnostic et l'artefact story-level documentent clairement pourquoi l'epic ne prend pas cet axe dans cette vague

**Dev notes :**
- story volontairement decoupee de `climate` car le risque produit et UX n'est pas du meme ordre
- aucune ouverture cosmétique autorisee

---

### Story 10.4 : Cadrage des composites metier `IQ EV` / `SPA` sans ouverture cosmetique

En tant que mainteneur,
je veux expliciter si `IQ EV` et `SPA` relevent d'un simple assemblage de types HA deja ouverts ou d'une vraie equivalence UX/metier supplementaire,
afin de ne pas confondre parite technique minimale et promesse produit excessive.

**Acceptance Criteria :**

**Given** les cas `IQ EV` et `SPA` remontes par l'audit Homebridge
**When** la story est executee
**Then** chacun est classe dans l'une des categories suivantes :
**And** `deja suffisamment couvert par composition de types existants` ;
**And** `necessite une extension bornee de type/de surface` ;
**And** `hors vague pe-epic-10`

**Given** un composite classe `deja suffisamment couvert`
**When** la story est close
**Then** la justification cite les entites HA deja suffisantes (`switch`, `sensor`, `binary_sensor`, eventuellement `climate`)
**And** aucune nouvelle promesse UX n'est ajoutee

**Given** un composite classe `necessite une extension`
**When** la story est close
**Then** elle ne force pas cette extension dans `pe-epic-10`
**And** elle produit un handoff clair vers `pe-epic-11+` ou un futur correct-course

**Dev notes :**
- story de cadrage, pas d'ouverture automatique
- `number` et `select` ne doivent etre introduits ici que si un besoin reel de consigne ou de mode distinct est prouve
- les zones sensibles (`presence`, energie, routage) restent sous gouvernance produit explicite
...
- `ha-projection-reference.md` reste citee ligne de preuve par ligne de preuve pour toute contrainte HA nouvelle ;
- toute ouverture de type dans `PRODUCT_SCOPE` respecte FR40 / NFR10 dans le meme increment ;
...
(livrée — multi-sensor eq553, 65 capteurs, gate terrain PASS box 192.168.1.21 ; Fix A éligibilité sans generic_type ; voir record `11-1-msunpv-routeursolaire-sensors-lecture-seule.md`)
...
### Story 11.2 — Chauffe-eau eq554 : détail routage
Livrée via PR #127 (`5e3d39b`). Projection multi-domaine de l'equipement eq554 : un `switch`, douze `sensor` et un `binary_sensor` sous un device commun, avec depublication domain-aware et persistance cross-reboot. Revue PASS (11/11 AC), 916 tests verts et gate terrain PASS le 2026-06-19 : quatorze entites publiees et `switch.jeedom2ha_554` non `unknown`.

### Story 11.3 — IQ EV Charger (eq583) + Pilotage priorisation solaire (eq628)
Livrée via PR #128 (`3546f22`). Le support generique `SWITCH_*`, le multi-switch structurel et le streaming `SWITCH_STATE` couvrent eq583 et eq628 sans allowlist d'identifiants. Gate terrain PASS le 2026-06-21 : eq583 publie trois `sensor`, deux `binary_sensor` et trois `switch`; eq628 publie quatre `switch`; tous les etats actionnables observes sont non `unknown`. Le controle de onze autres commandes `SWITCH_*` n'a revele aucune ouverture inattendue.
...
Livrée via PR #123 (`d5b86ce`, correctifs `6abfcf5` et `0a79966`). Le callback PHP, le resolver `resources/daemon/sync/state.py` et la publication MQTT fournissent l'etat initial puis les changements event-driven pour `sensor` et `binary_sensor`, uniquement quand l'entite est effectivement publiee en discovery. Gate terrain PASS le 2026-06-18 : 88 listeners actifs et le capteur eq553 « Tension reseau » expose 238.4 V au lieu de `unknown`; 860 tests verts.

### Story 12.2 — Streaming de valeur switch / button (vague 2)
Livrée via PR #125 (`6e24693`). Le chemin de valeur de 12.1 est etendu aux `switch`; les `button` restent volontairement sans etat readback. Gate terrain PASS le 2026-06-18 : commande switch reelle on/off, eq554 non `unknown`, listeners actifs et non-regression eq553/light/cover. Revue terminee apres correction d'un finding HIGH; 893 tests verts.
