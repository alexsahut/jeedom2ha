...
| Sprint Change Proposal le plus recent | `_bmad-output/planning-artifacts/sprint-change-proposal-2026-06-22-doc-realignment.md` |
| Delta Homebridge -> HA priorisant `pe-epic-10` | `_bmad-output/planning-artifacts/homebridge-homekit-vs-ha-delta-2026-06-07.md` |
...
Les epics `pe-epic-1` a `pe-epic-15` sont livres. Le cadrage historique de mapping configurable porte par la branche `chore/bmad-scp-epic-12` ne devait pas etre execute tel quel : le numero `pe-epic-12` etait consomme par le runtime state streaming. Ce sujet a ete repris et renumerote en `pe-epic-16` par le correct-course du 2026-07-06 (voir paragraphe ci-dessous) — c'est desormais la prochaine initiative produit engagee dans le tracker actif.

`pe-epic-13` (Energie HA exploitable) est desormais `done` (flip epic-level 2026-07-05). Correct-course du 2026-07-04 : ajout de `pe-epic-14` — Parite generique `FAN_*` -> `switch` (pompe filtration piscine, signalement ClawBox eq67/cmd382). Voir `sprint-change-proposal-2026-07-04-fan-switch-parity.md`. Scope Minor, extension pure de mapper sous le composant `switch` deja ouvert dans `PRODUCT_SCOPE` — aucune gouvernance FR40/NFR10 supplementaire requise. `pe-epic-14` est `done`.

Correct-course du 2026-07-05 : ajout de `pe-epic-15` — Visibilite console des capacites daemon (Energy `state_class`, streaming, FAN parity). Voir `sprint-change-proposal-2026-07-05-epic-visibilite-console.md`. `pe-epic-15` est `done`, 3/3 stories, gate terrain de cloture d'epic PASS le 2026-07-05.
...
Cycle actif = **Moteur de projection explicable**. Sources de verite actives = `product-brief-jeedom2ha-2026-04-09.md`, `prd.md`, `architecture-projection-engine.md`, `architecture-delta-review-prd-final.md`, `epics-projection-engine.md`, `implementation-readiness-report-2026-04-10.md`, `sprint-status.yaml`, `backlog-icebox.md`, `ha-projection-reference.md` et `sprint-change-proposal-2026-07-06-mapping-configurable.md`. Les epics `pe-epic-1` a `pe-epic-15` sont livres. `pe-epic-16` (mapping configurable commande par commande, renumerotation du cadrage historique `pe-epic-12`) est cadre par le SCP du 2026-07-06 et devient le prochain epic execute. Les artefacts `V1.1 Pilotable` sont du contexte secondaire uniquement. Aucun agent ne doit deviner la source de verite.
