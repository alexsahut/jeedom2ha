...
Les epics `pe-epic-10`, `pe-epic-11` et `pe-epic-12` sont livres. La prochaine initiative produit n'est pas encore engagee dans le tracker actif. Le cadrage historique de mapping configurable porte par la branche `chore/bmad-scp-epic-12` ne doit pas etre execute tel quel : le numero `pe-epic-12` est desormais consomme par le runtime state streaming. Si ce sujet est repris, il doit faire l'objet d'un nouveau correct-course depuis `main`, avec une nouvelle numerotation et une validation du scope courant.

`pe-epic-13` (Energie HA exploitable) est en statut `in-progress` avec toutes ses stories `done` — cloture epic-level restante mais hors scope de ce paragraphe. Correct-course du 2026-07-04 : ajout de `pe-epic-14` — Parite generique `FAN_*` -> `switch` (pompe filtration piscine, signalement ClawBox eq67/cmd382). Voir `sprint-change-proposal-2026-07-04-fan-switch-parity.md`. Scope Minor, extension pure de mapper sous le composant `switch` deja ouvert dans `PRODUCT_SCOPE` — aucune gouvernance FR40/NFR10 supplementaire requise.

## 10. Resume ultra court a copier dans les prompts

Cycle actif = **Moteur de projection explicable**. Sources de verite actives = `product-brief-jeedom2ha-2026-04-09.md`, `prd.md`, `architecture-projection-engine.md`, `architecture-delta-review-prd-final.md`, `epics-projection-engine.md`, `implementation-readiness-report-2026-04-10.md`, `sprint-status.yaml`, `backlog-icebox.md`, `ha-projection-reference.md` et `sprint-change-proposal-2026-06-22-doc-realignment.md`. Les epics `pe-epic-10`, `pe-epic-11` et `pe-epic-12` sont livres. Aucun prochain epic n'est engage; le mapping configurable historique doit etre renumerote et recadre depuis `main` avant usage. Les artefacts `V1.1 Pilotable` sont du contexte secondaire uniquement. Aucun agent ne doit deviner la source de verite.
