...
class ParsedTopic:
    ...
    eq_id: int
    channel: str  # set | brightness | position
...
class CommandTranslation:
    ...
    command_family: str  # on_off | cover_motion | slider
...
class CommandSynchronizer:
    ...
    def __init__(
        ...
        # Story 3.2 contract (validated on real box): cmd::execCmd uses core API key.
    ...
    def _expected_command_topics(self, mapping: MappingResult) -> Dict[str, str]:
        ...
        if mapping.ha_entity_type == "light":
            if bool(getattr(mapping.capabilities, "has_brightness", False)):
                topics["brightness"] = f"jeedom2ha/{eq_id}/brightness/set"

        if mapping.ha_entity_type == "cover":
            if bool(getattr(mapping.capabilities, "has_position", False)):
    ...
    def _is_state_sync_active(self, state_sync: Any) -> bool:
        """Return True only when Story 3.1 sync is really active/usable."""
