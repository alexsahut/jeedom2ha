...
def _sensor_mapping(eq_id: int = 4, *, binary: bool = False) -> MappingResult:
    generic_type = "PRESENCE" if binary else "TEMPERATURE"
    sub_type = "binary" if binary else "numeric"
    entity_type = "binary_sensor" if binary else "sensor"
    reason_code = "sensor_presence" if binary else "sensor_temperature"
    ...
    return MappingResult(
        ...
        jeedom_eq_id=eq_id,
        ha_unique_id=f"jeedom2ha_cmd_{eq_id}01",
        ha_name=f"Sensor {eq_id}",
        commands={
            generic_type: JeedomCmd(
                id=int(f"{eq_id}01"),
                name="State",
        ...
        capabilities=SensorCapabilities(is_binary=binary),
    )
...
@pytest.mark.asyncio
async def test_position_is_always_stateless_even_with_reliable_state():
    mapping = _cover_mapping(eq_id=3, has_stop=True, has_position=True, with_state_info=True)
    app = {
        "mappings": {mapping.ha_unique_id: mapping},
        "publications": {mapping.ha_unique_id: _decision(mapping)},
        "state_synchronizer": _FakeStateSynchronizer(active=True),
    ...
    ok = await sync.handle_command_message("jeedom2ha/3/position/set", "45")

    assert ok is True
    sync._execute_exec_cmd.assert_awaited_once_with(3004, {"slider": "45"})
    bridge.publish_message.assert_not_called()


@pytest.mark.asyncio
async def test_command_rejected_when_bridge_unavailable_before_rpc(caplog):
    mapping = _switch_mapping(eq_id=2, with_state_info=True)
    app = {
        "mappings": {mapping.ha_unique_id: mapping},
        "publications": {mapping.ha_unique_id: _decision(mapping)},
    ...
    assert ok is False
    sync._execute_exec_cmd.assert_not_awaited()
    assert "reason_code=mqtt_unavailable" in caplog.text
...
async def test_rpc_timeout_is_reported_and_never_publishes_immediate_state(caplog):
    ...
    async def _timeout_stub(cmd_id, options):  # noqa: ARG001 - test stub signature
...
async def test_sensor_set_topic_is_rejected_as_not_published_by_discovery(caplog, binary):
    mapping = _sensor_mapping(eq_id=4, binary=binary)
    app = {
        "mappings": {mapping.ha_unique_id: mapping},
        "publications": {mapping.ha_unique_id: _decision(mapping)},
        "state_synchronizer": _FakeStateSynchronizer(active=True),
