...
- La décision produit (étape 4, `publication_decision_ref`) reste intacte après
  toute issue de l'étape 5 (succès, échec technique, non-tentée).
...
async def test_publish_success_records_publication_result_success(cli, app):
    ...
    mapping = app["mappings"][201]
    decision = mapping.publication_decision_ref
...
async def test_not_attempted_when_decision_negative(cli, app):
    ...
    forced_negative_decision = PublicationDecision(
        should_publish=False,
        reason="low_confidence",
    )
    payload = _sync_body([_light_eq(204)])
    ...
    with patch("transport.http_server.DiscoveryPublisher", return_value=mock_publisher), patch(
        ...
        return_value=forced_negative_decision,
    ):
        resp = await cli.post("/action/sync", json=payload, headers={"X-Local-Secret": SECRET})
...
async def test_i2_invalid_projection_never_publishes_and_not_attempted(cli, app):
    """AC5/I2 — projection invalide → should_publish=False → publication_result='not_attempted'."""
    _set_connected_bridge(app)
    ...
    payload = _sync_body([_light_eq(206)])
    ...
    assert resp.status == 200
    mapping = app["mappings"][206]
    decision = mapping.publication_decision_ref

    # I2 : projection invalide → should_publish=False
    assert decision.should_publish is False
...
async def test_i4_cause_etape2_not_overwritten_by_etape5(cli, app):
    ...
    mock_publisher = AsyncMock()
    mock_publisher.publish_light = AsyncMock(return_value=True)
    forced_step4_decision = PublicationDecision(
        should_publish=False,
        reason="ambiguous_skipped",
    )
    payload = _sync_body([_light_eq(207)])

    with patch("transport.http_server.DiscoveryPublisher", return_value=mock_publisher), patch(
        "transport.http_server.decide_publication",
        return_value=forced_step4_decision,
    ):
        resp = await cli.post("/action/sync", json=payload, headers={"X-Local-Secret": SECRET})

    assert resp.status == 200
    mapping = app["mappings"][207]
    decision = mapping.publication_decision_ref
    assert decision is not None
...
async def test_failed_first_publish_does_not_trigger_cleanup_unpublish_on_next_sync(cli, app):
    ...
    mapping = app["mappings"][213]
    assert mapping.publication_result is not None
    assert mapping.publication_result.status == "failed"
    assert mapping.publication_decision_ref.discovery_published is False
